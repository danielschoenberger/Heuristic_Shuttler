...
from Circles import GraphCreator, MemoryZone, get_idx_from_idc
    ...
    for _k, seed in enumerate(seeds):
        ...
        while timestep < max_timesteps:
            ...
            for rotate_chain in move_sequence:
                ...
                ) or get_idx_from_idc(Mem1.idc_dict, next_edge) == get_idx_from_idc(
                    Mem1.idc_dict, Mem1.graph_creator.entry_edge
                ):
                    all_circles[rotate_chain] = [
                        edge_idc,
                        next_edge,
                        *Mem1.create_outer_circle(
                            edge_idc, next_edge, path_over_pz=True, include_path_to_exit_edge=False
                        ),
                        edge_idc,
                    ]
                else:
