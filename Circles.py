...
def get_path_to_node(nx_G, src, tar, exclude_pz=True):
    ...
    if exclude_pz == True:
        ...
        node_path = nx.shortest_path(nx_G, src, tar, lambda edge0, edge1, edge_attr_dict : (edge_attr_dict['edge_type'] == 'entry')*1e8+1)
    else:
...
def circle_is_contained_in_other_circle(subseq, seq):
    ...
    for i in range(seq_len):
        if seq_extended[i:i + subseq_len] == subseq:
            return True
...
class GraphCreator:
    ...
    def plot_state(self, ion_moves, plot_ions=True, show_plot=False):
        ...
        nx.draw_networkx(self.networkx_graph, pos=pos, with_labels=True, node_size=300, node_color=node_color, width=8, edge_color=edge_color, font_size=6)   
...
class MemoryZone():
    def __init__(self, m, n, v, h, starting_config, starting_sequence, max_timestep):          
        ...
        self.path_entry_to_exit = get_path_to_node(self.graph, self.graph_creator.entry, self.graph_creator.exit, exclude_pz=True)
    ...
    def find_next_edge(self, edge_idc):
        ...
        if get_idx_from_idc(self.idc_dict, edge_idc) == get_idx_from_idc(self.idc_dict, self.graph_creator.exit_edge):
            next_edge = self.graph_creator.entry_edge
            return next_edge
        elif get_idx_from_idc(self.idc_dict, edge_idc) == get_idx_from_idc(self.idc_dict, self.graph_creator.entry_edge):
            next_edge = [edge for edge in self.graph.edges(self.graph_creator.entry) if edge != self.graph_creator.entry_edge and edge != self.path_entry_to_exit[0]][0]
            # assert that next edge after entry is not entry or exit
            assert get_idx_from_idc(self.idc_dict, next_edge) != get_idx_from_idc(self.idc_dict, self.graph_creator.exit_edge)
            assert get_idx_from_idc(self.idc_dict, next_edge) != get_idx_from_idc(self.idc_dict, self.graph_creator.entry_edge)
            return next_edge

        # find shortest path from both sides for all other edges
        path0 = nx.shortest_path(self.graph, edge_idc[0], self.graph_creator.processing_zone, lambda edge0, edge1, edge_attr_dict : (edge_attr_dict['edge_type'] == 'entry')*1e8+1)
        path1 = nx.shortest_path(self.graph, edge_idc[1], self.graph_creator.processing_zone, lambda edge0, edge1, edge_attr_dict : (edge_attr_dict['edge_type'] == 'entry')*1e8+1)
        ...
        if len(path1) < len(path0):
            ...
            next_edge = get_idc_from_idx(self.idc_dict, next_edge_idx)
            return next_edge
        else:
    ...
    def find_ordered_edges(self, edge1, edge2):
        ...
        if len(common_node) != 1:
            raise ValueError("The input edges are not connected.")

        common_node = common_node.pop()
        if edge1[0] == common_node:
            edge1_in_order = (edge1[1], common_node)
            if edge2[0] == common_node:
                edge2_in_order = (common_node, edge2[1])
            else:
                edge2_in_order = (common_node, edge2[0])
        else:
    ...
    def have_common_junction_node(self, edge1, edge2):
        ...
        all_junctions = self.junction_nodes + [self.graph_creator.processing_zone] + [self.graph_creator.entry] + [self.graph_creator.exit]
    ...
    def create_outer_circle(self, edge_idc, next_edge, path_over_pz=False, include_path_to_exit_edge=True):
        ...
        assert edge_idc[1] == next_edge[0], 'edge_idc %s and next_edge %s are not connected' % (edge_idc, next_edge)
        if path_over_pz == False and include_path_to_exit_edge == False:
            raise ValueError('path_over_pz and include_path_to_exit_edge cannot both be False (no use case yet)')

        if path_over_pz == False:
            # if next edge is not in processing zone
            # create path that does not contain the chain -> forms circle around it
            node_path = nx.shortest_path(self.graph, next_edge[1], edge_idc[0], lambda node0, node1, edge_dic : [1e8 if (get_idx_from_idc(self.idc_dict, (node0, node1)) == get_idx_from_idc(self.idc_dict, edge_idc)
                                                                                                                                    or get_idx_from_idc(self.idc_dict, (node0, node1)) == get_idx_from_idc(self.idc_dict, next_edge)
                                                                                                                                    or get_idx_from_idc(self.idc_dict, (node0, node1)) == get_idx_from_idc(self.idc_dict, self.graph_creator.entry_edge))
                                                                                                                                    else 1][0])
        elif include_path_to_exit_edge == True:
            ...
            node_path = nx.shortest_path(self.graph, next_edge[1], edge_idc[0], lambda node0, node1, edge_dic : [1e8 if (get_idx_from_idc(self.idc_dict, (node0, node1)) == get_idx_from_idc(self.idc_dict, edge_idc)
                                                                                                                                    or get_idx_from_idc(self.idc_dict, (node0, node1)) == get_idx_from_idc(self.idc_dict, next_edge))
                                                                                                                                    or get_idx_from_idc(self.idc_dict, (node0, node1)) == get_idx_from_idc(self.idc_dict, self.path_entry_to_exit[0])
                                                                                                                                    else 1][0])
    ...
    def check_if_edge_is_filled(self, edge_idc):
        ...
        if get_idx_from_idc(self.idc_dict, edge_idc) in state_idxs:
            return True
        else:
            return False
    ...
    def find_circle_idxs_sharing_nodes(self, circles_dict):
        ...
        free_circle_combs = [circle_idx_pair for circle_idx_pair in combinations_of_circles if circle_idx_pair not in junction_shared_pairs]
    ...
    def rotate(self, full_circle_idx, plot=False):
        ...
        if plot == True:
            self.graph_creator.plot_state([get_idx_from_idc(self.idc_dict, edge_idc) for edge_idc in self.ion_chains.values()], show_plot=True)
    ...
    def find_next_unoccupied_edge(self, start_node):
        ...
        raise ValueError("No unoccupied edge found from the starting node.")
...
if __name__ == '__main__':
    ...
    if rand == True:
        ...
        starting_traps = [edges for edges in graph.edges() if graph.get_edge_data(edges[0], edges[1])['edge_type'] == 'trap'][:int(0.5*n_of_traps)+1]
    ...
    while timestep < max_timesteps:
        ...
        for i, rotate_chain in enumerate(sequence):
            ...
            if get_idx_from_idc(Mem1.idc_dict, edge_idc) == get_idx_from_idc(Mem1.idc_dict, Mem1.graph_creator.entry_edge):
        ...
        for rotate_chain in move_sequence:
            ...
            elif get_idx_from_idc(Mem1.idc_dict, next_edge) == get_idx_from_idc(Mem1.idc_dict, Mem1.graph_creator.exit_edge):               
            ...
            elif get_idx_from_idc(Mem1.idc_dict, next_edge) == get_idx_from_idc(Mem1.idc_dict, Mem1.graph_creator.entry_edge):
                # TODO
